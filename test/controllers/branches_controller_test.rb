...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference("Branch.count") do
      post branches_url, params: { branch: { location: @branch.location, name: @branch.name, string: @branch.name } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { location: @branch.location, name: @branch.name, string: @branch.name } }
